...
var Events = {
   ...
   var USERS      = ['get', 'set', 'create', 'restore', 'logout', '@verify', 'rate', 'bindSocial', 'stats', 'checkemail', 'support']
     , ORDERS     =  ['get', 'delete', 'create', 'update', 'leavecomment', 'getcomments', 'putSolution', 'getsolution', 'search']
   ...
   Q.prototype.api = function(method, params) {
    ...
    try {
     ...
     JSONP(reqURL, params, function(data){ 
       if(data && (data.response || !data.error)) d.resolve(data.response ? data.response : data, $this, method);
    ...
    forEach(USERS, function(index, value)  {
     USERS_API[value] = function(params) {
       ...
       if(!options.access_token) {
         if(window.app.access_token) params.access_token = window.app.access_token;
      }
   ...
   Q.prototype.orders = function(options) {
    var $this = this;
    options = options || {};
    forEach(ORDERS, function(index, value)  {
     ORDERS_API[value] = function(params) {
       params = params || {};
       if(!options.access_token) {
         if(window.app.access_token) params.access_token = window.app.access_token;
      }
       return $this.api('orders.' + value, params);
       }
     });
    return ORDERS_API;
   }
